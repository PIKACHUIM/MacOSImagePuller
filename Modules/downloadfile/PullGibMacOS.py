...
class gibMacOS:
    ...
    def set_prods(self, progressbar=None):
        self.resize()
        ...
        self.mac_prods = self.get_dict_for_prods(self.get_installers(), progressbar=progressbar)
    ...
    def num_to_macos(self, macos_num, for_url=True):
        if for_url:  # Resolve 8-5 to their names and show Big Sur as 10.16
            return self.mac_os_names_url.get(str(macos_num), "10.{}".format(macos_num)) if macos_num <= 16 else str(
                macos_num - 5)
    ...
    def build_url(self, **kwargs):
        ...
        return "https://swscan.apple.com/content/catalogs/others/index-{}.merged-1.sucatalog".format(
            "-".join(reversed(
                self.get_macos_versions(self.min_macos, version, catalog=self.catalog_suffix.get(catalog, ""))))
        )
    ...
    def get_installers(self, plist_dict=None):
        ...
        for p in plist_dict.get("Products", {}):
            if not self.find_recovery:
                val = plist_dict.get("Products", {}).get(p, {}).get("ExtendedMetaInfo", {}).get(
                    "InstallAssistantPackageIdentifiers", {})
                if val.get("OSInstall", {}) == "com.apple.mpkg.OSInstall" or val.get("SharedSupport", "").startswith(
                        "com.apple.pkg.InstallAssistant"):
                    mac_prods.append(p)
            else:
                # Find out if we have any of the recovery_suffixes
                if any(x for x in plist_dict.get("Products", {}).get(p, {}).get("Packages", []) if
                       x["URL"].endswith(self.recovery_suffixes)):
                    mac_prods.append(p)
        return mac_prods
    ...
    def get_dict_for_prods(self, prods, plist_dict=None, progressbar=None):
        ...
        for prod in prods:
            if progressbar is not None:
                progressbar['value'] = count / len(prods) * 10
                count += 1
            ...
            prodd["installer"] = plist_dict.get("Products", {}).get(prod, {}).get("ExtendedMetaInfo", {}).get(
                "InstallAssistantPackageIdentifiers", {}).get("OSInstall", {}) == "com.apple.mpkg.OSInstall"
            ...
            if self.find_recovery:
                ...
                prodd["packages"] = [x for x in plist_dict.get("Products", {}).get(prod, {}).get("Packages", []) if
                                     x["URL"].endswith(self.recovery_suffixes)]
            else:
                # Add them all!
    ...
    def download_prod(self, prod, dmg=False, progressbar=None):
        # Takes a dictonary of details and downloads it
        self.resize()
        name = "{} - {} {} ({})".format(prod["product"], prod["version"], prod["title"], prod["build"]).replace(":",
                                                                                                                "").strip()
        download_dir = self.download_dir or os.path.join(os.path.dirname(os.path.realpath(__file__)), "macOS Downloads",
    ...
    def main(self, dmg=False, progressbar=None):
        lines = []
        ...
        lines.append(" ")
        lines.append(
            "M. Change Max-OS Version (Currently {})".format(self.num_to_macos(self.current_macos, for_url=False)))
    ...
    def get_for_version(self, vers, build=None, device_id=None, dmg=False, progressbar=None):
        ...
        for p in sorted(self.mac_prods, key=lambda x: x['version'], reverse=True):
            ...
            if not pv in [" ", "1.0"]:
                ...
                if pv.startswith(v):
                    self.download_prod(p, dmg, progressbar=progressbar)
                    return
...
if __name__ == '__main__':
    ...
    parser.add_argument("-l", "--latest",
                        help="downloads the version available in the current catalog (overrides --build, --version and --product)",
                        action="store_true")
    parser.add_argument("-r", "--recovery",
                        help="looks for RecoveryHDUpdate.pkg and RecoveryHDMetaDmg.pkg in lieu of com.apple.mpkg.OSInstall (overrides --dmg)",
                        action="store_true")
    ...
    parser.add_argument("-n", "--newlocal",
                        help="downloads and saves locally, overwriting any prior local sucatalog.plist",
                        action="store_true")
    ...
    parser.add_argument("-b", "--build",
                        help="sets the build of macOS to target - eg '22G120' (must be used together with --version)")
    ...
    parser.add_argument("-D", "--device-id",
                        help="use with --version or --latest to search for versions supporting the specified Device ID - eg VMM-x86_64 for any x86_64")
    parser.add_argument("-i", "--print-urls", help="only prints the download URLs, does not actually download them",
                        action="store_true")
    parser.add_argument("-j", "--print-json",
                        help="only prints the product metadata in JSON, does not actually download it",
                        action="store_true")
    parser.add_argument("--no-interactive",
                        help="run in non-interactive mode (auto-enabled when using --product or --version)",
                        action="store_true")
