...
INFO_SIGN_HASH = 'CH'
INFO_SIGN_SESS = 'CT'
INFO_REQURED = [INFO_PRODUCT, INFO_IMAGE_LINK, INFO_IMAGE_HASH, INFO_IMAGE_SESS, INFO_SIGN_LINK, INFO_SIGN_HASH,
                INFO_SIGN_SESS]
...
def verify_chunklist(cnkpath):
    with open(cnkpath, 'rb') as f:
        ...
        magic, header_size, file_version, chunk_method, signature_method, chunk_count, chunk_offset, signature_offset = ChunkListHeader.unpack(
            data)
        assert magic == b'CNKL'
        ...
        if signature_method == 1:
            ...
            plaintext = int(f'0x1{"f" * 404}003031300d060960864801650304020105000420{"0" * 64}', 16) | int.from_bytes(
                digest, 'big')
...
def save_image(url, sess, filename='', directory='', process=None, start=0):
    purl = urlparse(url)
    headers = {
        'Host': purl.hostname,
        'Connection': 'close',
...
def verify_image(dmgpath, cnkpath, process=None):
...
def action_download(args, process=None):
    """
    ...
    try:
        if not process['execute']:
            return False
        verify_image(dmgpath, cnkpath, process)
        process['process'] = 100
        return 0
...
def action_verify(args):
    ...
    uproduct_default = get_image_info(session, bid=args.board_id, mlb=product_mlb(args.mlb), diag=False,
                                      os_type='default')
    ...
    if uvalid_default[INFO_PRODUCT] != uvalid_latest[INFO_PRODUCT]:
        print(f'SUCCESS: {args.mlb} MLB looks valid and supported!' if uvalid_latest[INFO_PRODUCT] == generic_latest[
            INFO_PRODUCT] else f'SUCCESS: {args.mlb} MLB looks valid, but probably unsupported!')
        return 0
